...
<html>
   <meta http-equiv='Content-Type' content='text/html; charset=utf-8'>
    ...
    <center>
        <iframe id='ytplayer' type='text/html' width='75%' height='75%'
...
def get_video_urls_from_page(start, end):
    ...
    for page_num in range(start, end + 1):
        raw_page = urllib.urlopen(BASE_URL + str(page_num) + '.json').read()
        page_data = json.loads(raw_page)
        thread_numbers = [t['posts'][0]['no'] for t in page_data['threads']]
        for no in thread_numbers:
            raw_t = urllib.urlopen(BASE_URL + '/thread/' + str(no) + '.json').read()
